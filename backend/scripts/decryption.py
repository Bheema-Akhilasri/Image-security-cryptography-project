...
#     """
#     key: numpy array (uint8, 0–255) generated by DL model
#     returns: x0 in (0,1), r in (3.57, 4)
#     """
#     x0 = key[0] / 255.0
...
#     # 🔑 Get x0, r from DL key (ONLY CHANGE)
#     x0, r = key_to_chaos_params(key)

#     chaotic_seq = logistic_map(x0, r, num_pixels)

#     # ---------- STEP 1: Reverse XOR (UNCHANGED) ----------
#     chaotic_vals = (chaotic_seq * 255).astype(np.uint8)
#     chaotic_vals = np.repeat(chaotic_vals[:, np.newaxis], 3, axis=1)
...
#     # ---------- STEP 2: Reverse permutation (UNCHANGED) ----------
#     shuffle_indices = np.argsort(chaotic_seq)
#     inverse_perm = np.argsort(shuffle_indices)
...
# ---------------------------------
# 🔹 Logistic Map (same as encryption)
# ---------------------------------
def logistic_map(x0, r, size):
    seq = np.zeros(size)
    x = x0

    for _ in range(100):  # stabilization
        x = r * x * (1 - x)
...
def decrypt_image(input_path, output_path, x0, r):
    ...
    # ---------------------------------
    # 🔁 Prepare chaotic values
    # ---------------------------------
    chaotic_vals = (chaotic_seq * 255).astype(np.uint8)
    chaotic_vals = chaotic_vals.reshape(num_pixels, 1)
    chaotic_vals = np.repeat(chaotic_vals, 3, axis=1)

    flat_enc = enc_array.reshape(num_pixels, c)

    # ---------------------------------
    # 🔁 Reverse diffusion
    # ---------------------------------
    decrypted = np.zeros_like(flat_enc)

    prev = np.array([0, 0, 0], dtype=np.uint8)

    for i in range(num_pixels):
        # reverse of: enc = data ^ chaos ^ prev
        decrypted[i] = flat_enc[i] ^ chaotic_vals[i] ^ prev
        prev = flat_enc[i]   # IMPORTANT: use encrypted value here
    ...
    reverse_perm = np.zeros_like(perm_indices)
    reverse_perm[perm_indices] = np.arange(num_pixels)

    unshuffled = decrypted[reverse_perm]

    # reshape back
    final_img = unshuffled.reshape(h, w, c)
